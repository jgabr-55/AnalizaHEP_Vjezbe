...
void Analyzer::Loop(TString fileName)
{
	...
	TCanvas *c = new TCanvas("c3","c3",1800,1800); // brojevi se odnose na dimenzije slike
...
void Analyzer::fitHiggs()
{
	...
	polinom = new TF1("polinom","([0]+[1]*x+[2]*x*x)",110,150);
	suma = new TF1("suma","([0]*[1])/((x*x-[2]*[2])*(x*x-[2]*[2])+0.25*[1]*[1]) + [3]+[4]*x+[5]*x*x",110,150);
	bw->SetParameters(2.04150e+04,8.29916e+02,1.24443e+02);
	polinom->SetParameters(5.18016e+01,-6.21281e-01,2.45073e-03);
	suma->SetParameters(2.04150e+04,8.29916e+02,1.24443e+02,5.18016e+01,-6.21281e-01,2.45073e-03);
	...
	c2->cd(2);
	h5_higgs->Add(h5_qqzz);
	h5_higgs->GetXaxis()->SetRangeUser(110.0,150.0);
	h5_higgs->Fit(suma,"L");

	h5_higgs->Draw();
	gStyle->SetOptFit();
	c2->SaveAs("higgs_fit.png");
}

void Analyzer::fitHiggsAndZ()
{
	TCanvas *c2 = new TCanvas("c2","c2",900,900);
	bw_higgs = new TF1("BW","([0]*[1])/((x*x-[2]*[2])*(x*x-[2]*[2])+0.25*[1]*[1])",70,170);
	polinom = new TF1("polinom","([0]+[1]*x+[2]*x*x)",70,170);
	bw_z=new TF1("BWz","([0]*[1])/((x*x-[2]*[2])*(x*x-[2]*[2])+0.25*[1]*[1])",70,170);

	suma = new TF1("suma","([0]*[1])/((x*x-[2]*[2])*(x*x-[2]*[2])+0.25*[1]*[1]) + [3]+[4]*x+[5]*x*x + ([6]*[7])/((x*x-[8]*[8])*(x*x-[8]*[8])+0.25*[7]*[7])",70,170);
	

	bw_higgs->SetParameters(2.04150e+04,8.29916e+02,1.24443e+02);
	bw_z->SetParameters(4.27919e+04,8.90531e+02,9.08734e+01);
	polinom->SetParameters(5.18016e+01,-6.21281e-01,2.45073e-03);
	suma->SetParameters(2.04150e+04,8.29916e+02,1.24443e+02,5.18016e+01,-6.21281e-01,2.45073e-03,4.27919e+04,8.90531e+02,9.08734e+01);
	...
	//c2->cd(2);
	h5_higgs->Add(h5_qqzz);
	h5_higgs->GetXaxis()->SetRangeUser(70.0,170.0);
	h5_higgs->Fit(suma,"L");

	h5_higgs->Draw();
	gStyle->SetOptFit();
	c2->SaveAs("zad2_fit_sve.png");
